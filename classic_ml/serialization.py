...
def save_object(obj, filepath, compress=True):
    """
    Function to serialize and optionally compress a Python object from memory for storage into a local binary file
...
def load_object(filepath, compressed=True):
    """
    Function to deserialize (and decompress) a Python object from local storage and load it into memory
